...
struct block *findFreeBlock(struct block **last, size_t size) 
{
   ...
   /* Best fit */
   struct block *next_block_best = FreeList;
   /* Find the first free block B1 that can hold the process P1.
      After that move another pointer and check for free block
      that is large enough to hold the process but smaller than block B1 */
   while (curr && !(curr->free && curr->size >= size)) 
   {
      *last = curr;
      curr  = curr->next;
   ...
   if (curr) 
   {
     next_block_best = curr->next;
     while (next_block_best != NULL) 
     {
        while (next_block_best->free && next_block_best->size >= size && next_block_best->size < curr->size)
        {
          curr = next_block_best;
        }
        *last = next_block_best;
        next_block_best = next_block_best->next;
     }
   }
#endif
   ...
   /* Worst fit */
   struct block *next_block_worst = FreeList;
   while (curr && !(curr->free && curr->size >= size)) 
   {
      *last = curr;
      curr  = curr->next;
   ...
   if (curr) 
   {
     next_block_worst = curr->next;
     while (next_block_worst != NULL) 
     {
        while (next_block_worst->free && next_block_worst->size >= size && next_block_worst->size > curr->size)
        {
          curr = next_block_worst;
        }
        *last = next_block_worst;
        next_block_worst = next_block_worst->next;
     }
   }
#endif
...
void *malloc(size_t size) 
{
   ...
   {
     struct block *new_block = (struct block *)sbrk(sizeof(struct block));
     new_block->free = true;
     new_block->size = next->size - size;
     new_block->next = next->next;
     next->next = new_block;
     num_splits++;
     next->size = size; //making the current size to requested size
     num_blocks += 1;
   }
...
void free(void *ptr) 
{
   ...
   if(curr->next != NULL)
   {
     while (curr->next->free)
     {
      ...
      num_coalesces++;
      num_blocks -= 1; // number of blocks decreases while coalescing
      if(curr->next == NULL)
      {
        break;
      }
